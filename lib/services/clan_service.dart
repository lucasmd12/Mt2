...
class ClanService with ChangeNotifier {
 ...
 try {
  ...
  Future<ClanWar?> declareWar(String challengedClanId, String rules) async {
    final currentUser = _authService.currentUser;
    ...
    if (response != null && response['success'] == true) {
      return ClanWar.fromMap(response['data']);
    }
    return null;
  }

  Future<ClanWar?> acceptWar(String warId) async {
  ...
  Future<ClanWar?> rejectWar(String warId) async {
  ...
  Future<ClanWar?> reportWarResult(String warId, String winnerClanId, String loserClanId, Map<String, int> score) async {
    final response = await _apiService.post(
  ...
  Future<ClanWar?> cancelWar(String warId, String reason) async {
  ...
  Future<List<ClanWar>> getActiveWars() async {
    final response = await _apiService.get('/api/clan-wars/active', requireAuth: true);
    if (response != null && response['success'] == true && response['data'] is List) {
      return (response['data'] as List).map((json) => ClanWar.fromMap(json)).toList();
    }
    return [];
  }
}
